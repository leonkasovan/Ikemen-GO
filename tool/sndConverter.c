...
unsigned char* wav_to_mp3(const unsigned char* data, size_t len, size_t* outLen) {
    return convert_generic(data, len, outLen, SF_FORMAT_MPEG | SF_FORMAT_MPEG_LAYER_III, "mp3");
}
...
unsigned char* wav_to_adpcm(const unsigned char* data, size_t len, size_t* outLen) {
    return convert_generic(data, len, outLen, SF_FORMAT_WAV | SF_FORMAT_IMA_ADPCM, "wav");
}

unsigned char* wav_to_flac(const unsigned char* data, size_t len, size_t* outLen) {
    return convert_generic(data, len, outLen, SF_FORMAT_FLAC | SF_FORMAT_PCM_24, "flac");
}
...
int main(int argc, char** argv) {
    ...
    if (argc < 4) {
        ...
        printf("Modes:\n");
        printf("  --mp3    (MPEG Layer III)\n");
        printf("  --ogg    (Vorbis)\n");
        printf("  --flac   (Free Lossless Audio Codec)\n");
        printf("  --adpcm  (IMA ADPCM WAV)\n");
        return 1;
    }
    ...
    if (strcmp(argv[1], "--mp3") == 0)        { mode = MODE_MP3; modeName = "MP3"; }
    else if (strcmp(argv[1], "--ogg") == 0)   { mode = MODE_OGG; modeName = "OGG"; }
    else if (strcmp(argv[1], "--flac") == 0)  { mode = MODE_FLAC; modeName = "FLAC"; }
    else if (strcmp(argv[1], "--adpcm") == 0) { mode = MODE_ADPCM; modeName = "ADPCM"; }
    else {
    ...
    for (uint32_t i = 0; i < hdr.cSounds; i++) {
        ...
        switch (mode) {
            case MODE_MP3:    newData = wav_to_mp3(wavData, sub.cbSubfile, &newSize); break;
            case MODE_OGG:    newData = wav_to_ogg(wavData, sub.cbSubfile, &newSize); break;
            case MODE_FLAC:   newData = wav_to_flac(wavData, sub.cbSubfile, &newSize); break;
            case MODE_ADPCM:  newData = wav_to_adpcm(wavData, sub.cbSubfile, &newSize); break;
        }

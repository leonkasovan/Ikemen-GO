...
type Mixer struct {
	streamers     []Streamer
	stopWhenEmpty bool
	// tmp is a pre-allocated mixing buffer reused across Stream calls to avoid
	// per-call stack allocation of a 512-sample (8 KB) frame, which is
	// especially costly on aarch64 where large stack frames trigger morestack.
	tmp [512][2]float64
}

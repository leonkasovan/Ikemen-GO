...
type decoder struct {
	...
	pos     int32
	err     error
	readBuf []byte // reused across Stream calls to avoid per-call allocation
}
...
func (d *decoder) Stream(samples [][2]float64) (n int, ok bool) {
	...
	// Grow readBuf only when needed; never shrink so it can be reused on
	// subsequent Stream calls without allocating.
	if cap(d.readBuf) < numBytes {
		d.readBuf = make([]byte, numBytes)
	}
	p := d.readBuf[:numBytes]
	n, err := d.r.Read(p)

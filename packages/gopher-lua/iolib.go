...
func fileIsWritable(L *LState, file *lFile) int {
	if file.writer == nil {
		L.Push(LNil)
		L.Push(LString(fmt.Sprintf("%s is opened for only reading.", "file")))
...
func fileIsReadable(L *LState, file *lFile) int {
	if file.reader == nil {
		L.Push(LNil)
		L.Push(LString(fmt.Sprintf("%s is opened for only writing.", "file")))
		L.Push(LNumber(1)) // C-Lua compatibility: Original Lua pushes errno to the stack
		return 3
	}
	return 0
...
func fileCloseAux(L *LState, file *lFile) int {
	// fmt.Printf("[iolib.go] fileCloseAux file=%v file.fp=%v\n", file, file.fp)
	file.closed = true
	var err error
	if file.writer != nil {
	...
	for i := idx; i <= top; i++ {
		...
		case LString:
			...
			for _, opt := range options[1:] {
				...
				case 'a':
					var buf []byte
					// fmt.Printf("[iolib.go] fileReadAux file=%v file.fp=%v\n", file, file.fp)
					buf, err = physfs.ReadAll(file.fp)
					// buf, err = file.reader.Peek(file.reader.Buffered())
					if err == io.EOF {
						L.Push(emptyLString)
						goto normalreturn
					}

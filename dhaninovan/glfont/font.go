...
func (f *Font) UpdateResolution(windowWidth int, windowHeight int) {
	...
	resUniform := gl.GetUniformLocation(f.program, "resolution")
	gl.Uniform2f(resUniform, float32(windowWidth), float32(windowHeight))
	gl.UseProgram(gl.Program{Value: 0})
}
...
func (f *Font) Printf(x, y float32, scale float32, align int32, blend bool, window [4]int32, fs string, argv ...interface{}) error {
	...
	for i := range indices {
		...
		//BufferSubData(target Enum, offset int, data []float32)
		gl.BufferSubDataFloat(gl.ARRAY_BUFFER, 0, vertices) // Be sure to use glBufferSubData and not glBufferData
		// Render quad
